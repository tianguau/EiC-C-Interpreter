...
char *Version =
"*************************************************************************\n"
"EiC V4.4.3 - Copyright (c) 2009,"
" by Aki Niimura\n"
"EiC V4.3.3 - Copyright (c) 1995 to 2000,"
" by Edmond J. Breen\n"
...
void EiC_usr2_int(int i)
{
    signal(SIGUSR2, EiC_usr2_int);
    errStr("EiC User Signal 2");
    if(jmpOn && jmpOn++ <= 1)
      longjmp(env,Err_);
    else
      eic_disaster();
...
FILE *EiChist = NULL;
extern int EiC_verboseON, EiC_SHOWLINE, EiC_traceON, EiC_traceFunc, EiC_TIMER,
...
void usage(void)
{
	...
	/* creating another copy of the message */
	while((cp = strstr(cp, "eic")) != NULL) {
	  strncpy(cp, "eim", 3);
	  cp += strlen("eim");
	}
	fputs(__backward__ ? msg : altmsg, stdout);
	free(altmsg);
	...
	puts(__backward__ ? "" :
	 ...
	 "\t-x -X       deferred link2eic initialization option\n"
	 );
    exit(1);
}
