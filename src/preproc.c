...
static void expr_list(eicstack_t *parms, char **p,int more)
{
	...
	for (; *s; ++s) {
		...
		s--;
            ...
            /* ISSUE reported by delyn (07242009): when c == sz-1 and s*='\\' 
               we need to put 2 chars but only one is remaining in str 
...
extern char *EiC_nextproline(void)
{
    ...
    if (EiC_Infile->fd == STDIN)
	out(EOF);	/* only EOF in the oline */
    else if (EiC_SHOWLINE && !showON) {
	out('\0');
	fputs(oline,stdout);
	fputc('\n',stdout);
    }
